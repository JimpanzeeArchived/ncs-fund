...
//get node identifier for buttion 1 (alias sw0)


/* STEP 3.2 - Get the device pointer. pin number, and pin's configuration flags through gpio_dt_spec */
static const struct gpio_dt_spec button = GPIO_DT_SPEC_GET(SW0_NODE, gpios);
/*gpio_dt_spec struct button now contains values:
1) Device pointer
2) Pin Number
3) Pin's Flag*/

/* LED0_NODE is the devicetree node identifier for the "led0" alias. */
#define LED0_NODE DT_ALIAS(led0) //get node identifier for LED0
static const struct gpio_dt_spec led = GPIO_DT_SPEC_GET(LED0_NODE, gpios);
//gpio_dt_spec struct LED now contains values with parameters from LED.
